...
pub struct KeyboardInfo {
    ...
    pub default_hotkey: Option<String>,  // Default hotkey from .km2 file
    pub hotkey: Option<String>,          // Custom hotkey set by user
    pub enabled: bool,
}
...
impl KeyboardManager {
    ...
    pub fn load_keyboard(&mut self, path: &Path) -> Result<String> {
        ...
        let info = KeyboardInfo {
            ...
            hotkey: None,  // No custom hotkey set initially
            enabled: true,
        };
            ...
            if RegOpenKeyExW(HKEY_CURRENT_USER, key_path, 0, KEY_READ, &mut hkey) .is_ok() {
                ...
                loop {
                    ...
                    if RegOpenKeyExW(HKEY_CURRENT_USER, PCWSTR::from_raw(kb_key_path_w.as_ptr()), 0, KEY_READ, &mut kb_hkey) .is_ok() {
                        ...
                        // Try to load default hotkey from .km2 file
                        let path_buf = PathBuf::from(&path);
                        let default_hotkey = if path_buf.exists() {
                            if let Ok(km2_data) = std::fs::read(&path_buf) {
                                if let Ok(km2) = Km2Loader::load(&km2_data) {
                                    km2.metadata().hotkey().map(|s| s.to_string())
                                } else {
                                    None
                                }
                            } else {
    ...
    pub fn get_effective_hotkey(&self, info: &KeyboardInfo) -> Option<String> {
        // Return custom hotkey if set, otherwise default hotkey
        info.hotkey.clone().or_else(|| info.default_hotkey.clone())
    }
    
    pub fn set_keyboard_hotkey(&mut self, keyboard_id: &str, hotkey: &str) -> Result<()> {
        if let Some(info) = self.keyboards.get_mut(keyboard_id) {
            info.hotkey = if hotkey.is_empty() { None } else { Some(hotkey.to_string()) };
        ...
        unsafe {
            ...
            if enabled {
                ...
                if RegOpenKeyExW(
                    ...
                    self.write_registry_string(hkey, w!("KeyMagic"), &exe_path)?;
                    RegCloseKey(hkey);
                }
            } else {
                ...
                if RegOpenKeyExW(
                    ...
                    RegDeleteValueW(hkey, w!("KeyMagic"));
                    RegCloseKey(hkey);
                }
            }
            
            // Also save this preference in our settings
            if RegCreateKeyW(
                HKEY_CURRENT_USER,
                w!("Software\\KeyMagic\\Settings"),
                &mut hkey
            ).is_ok() {
                self.write_registry_dword(hkey, w!("StartWithWindows"), if enabled { 1 } else { 0 })?;
                RegCloseKey(hkey);
            }
        }
