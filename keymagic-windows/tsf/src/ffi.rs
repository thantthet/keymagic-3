//! FFI bridge for Windows TSF integration
//! 
//! This module provides C-compatible functions that can be called from
//! the C++ TSF implementation to interact with the Rust KeyMagic core.

use keymagic_core::{KeyMagicEngine, KeyInput};
use keymagic_core::engine::{ModifierState, ActionType, Predefined};
...
/// Creates a new engine instance (uninitialized)
#[no_mangle]
            ...
            if let Some(engine) = engine_opt.as_mut() {
                match engine.process_key(key_input) {
                    Ok(result) => {
                        ...
                        // Process action and determine if key was consumed
                        let consumed = match &result.action {
                            ActionType::None => {
                                output_ref.action_type = 0;
                                // Key is consumed if composing text exists (part of composition)
                                !result.composing_text.is_empty()
                            }
                            ...
                            ActionType::BackspaceDeleteAndInsert(count, text) => {
                                ...
                                true // Always consumed if modifying
                            }
                        };
                        
                        output_ref.consumed = if consumed { 1 } else { 0 };
